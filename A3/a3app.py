...
class ColorWidget(BoxLayout):
    ...
    def on_kv_post(self, widget):
        ...
        if self.gammPanel and self.gammPanel.slider:
            self.gammPanel.slider.bind(value=self.recalibrate)
        self.recolor()
    ...
    def recalibrate(self,instance,value):
        """
        Recalibrates the gamma panel to use the new color and gamma value.
        
        This can be called for two different events: a change in color or a change
        in gamma. On a change in color, instance and value will be None, but 
        ...
        """
        if not self.gammPanel.slider:
            return
        
        origRGB = introcs.RGB(self.rgb.red,self.rgb.green,self.rgb.blue)
        gamma = self.gammPanel.slider.value
        gRGB  = a3.rgb_gamma(self.rgb,gamma)
        if self.rgb != origRGB:
            raise RuntimeError('rgb_gamma modified the color object')
        elif gRGB is None:
            gRGB = origRGB
        gComp = a3.complement_rgb(gRGB)
        self.gammPanel.foreground = gComp.glColor()
        self.gammPanel.background = gRGB.glColor()
        
        if self.rightSep:
            if self.mainPanel:
                self.rightSep.left  = self.mainPanel.background
            if self.gammPanel:
                self.rightSep.right = self.gammPanel.background
