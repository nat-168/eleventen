...
class Board:
    ...
    def isFullColumn(self,col):
        ...
        return self.findAvailableRow(col) == self.getHeight()
    ...
    def place(self,column,color):
        ...
        assert 0 <= column < self.getWidth()

        assert isinstance(color, str)
        assert introcs.is_tkcolor(color) or introcs.is_webcolor(color)


        if self.isFullColumn(column) == True:
            return -1

        posPlaceRow = self.findAvailableRow(column)
        self._board[posPlaceRow][column] = color

        self._moves.append((posPlaceRow, column))
    ...
    def undoPlace(self):
        ...
        if len(self._moves) > 0:
            removerow = self._moves[-1][0]
            removecolumn = self._moves[-1][1]

            self._board[removerow][removecolumn] = NOBODY

            self._moves = self._moves[:len(self._moves)-1]
...
#### TASK 4 ####
    ...
    def findAcross(self,r,c,leng):
        ...
        while c1 >= 0 and self._board[r][c1] == rc:
            c1 = c1 - 1
        c1 = c1 + 1

        while c2 < self.getWidth() and self._board[r][c2] == rc:
            c2 = c2 + 1
        c2 = c2 - 1
    ...
    def findSWNE(self,r,c,leng):
        ...
        while c1 >= 0 and r1 >= 0 and self._board[r1][c1] == rc:
            c1 = c1 - 1
            r1 = r1 - 1
        c1 = c1 + 1
        r1 = r1 + 1

        while c2 < self.getWidth() and r2 < self.getHeight() and self._board[r2][c2] == rc:
            c2 = c2 + 1
            r2 = r2 + 1
        c2 = c2 - 1
        r2 = r2 - 1
    ...
    def findNWSE(self,r,c,leng):
        ...
        while c1 >= 0 and r1 < self.getHeight() and self._board[r1][c1] == rc:
            c1 = c1 - 1
            r1 = r1 + 1
        c1 = c1 + 1
        r1 = r1 - 1

        while c2 < self.getWidth() and r2 >= 0 and self._board[r2][c2] == rc:
            c2 = c2 + 1
            r2 = r2 - 1
        c2 = c2 - 1
        r2 = r2 + 1
    ...
    def findWins(self,r,c):
        ...
        lengw = self.getStreak()
        if self.findVertical(r, c, lengw) is not None:
            return self.findVertical(r, c, lengw)

        if self.findAcross(r, c, lengw) is not None:
            return self.findAcross(r, c, lengw)

        if self.findSWNE(r, c, lengw) is not None:
            return self.findSWNE(r, c, lengw)

        if self.findNWSE(r, c, lengw) is not None:
            return self.findNWSE(r, c, lengw)
